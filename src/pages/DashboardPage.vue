...
const view = ref(true);
const handleClick = () => {
  // Ejemplo: cambiar el ancho al hacer click
  view.value = !view.value;
  widthSideBar.value = view.value ? "300px" : "0px";
};
</script>
<template>
  <div class="flex h-screen">
    <div
      class="bg-blue-950 h-full transition-all ease-in-out duration-100 overflow-hidden"
      :style="{ width: widthSideBar }"
    >
      <div
        class="flex flex-col gap-5 py-5 px-7 text-white text-[0.90rem] font-semibold"
      >
        ...
        <div>Dashboard</div>
        <div class="text-lg text-slate-500 font-bold">Mis procesos</div>
        <div>Casos vigentes</div>
        <div>Historial</div>

        <div class="text-lg text-slate-500 font-bold">Agenda</div>
        <div>Calendario</div>
      </div>
    </div>
